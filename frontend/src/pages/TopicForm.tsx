...
export function TopicForm() {
  ...
  const [form, setForm] = useState({ project_id: '', name: '', description: '', status: 'ativo' });
  const [formError, setFormError] = useState('');
  const load = useCallback(async () => {
    const [projects, topic] = await Promise.all([
      api.get<Project[]>('/projects'),
      isEdit ? api.get<Topic>(`/topics/${id}`) : Promise.resolve(null),
    ]);
  ...
  useEffect(() => {
    if (data?.topic) {
      setForm({
        project_id: data.topic.project_id ? String(data.topic.project_id) : '',
  ...
  async function submit(event: FormEvent) {
    ...
    if (isEdit) {
      await api.put<Topic>(`/topics/${id}`, payload);
    } else {
